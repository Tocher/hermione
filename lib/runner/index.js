...
module.exports = class MainRunner extends QEmitter {
    ...
    _runTestsInBrowser(files, browserId) {
        ...
        const fileRunner = FileRunner.create(browserAgent, this._config);
        ...
        return _(files)
            .map((file) => fileRunner.run(file))
            .thru(utils.waitForResults)
            .value();
    }

    // _runTestsInBrowser(files, browserId) {
    //     const runner = BrowserRunner.create(browserId, this._browserPool, this._config);
    //
    //     qUtils.passthroughEvent(runner, this, _.values(RunnerEvents.getSync()));
    //
