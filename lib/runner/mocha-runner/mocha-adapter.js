...
module.exports = class MochaAdapter {
    ...
    _injectBeforeHookErrorHandling() {
        this._injectHookErrorHandling('beforeAll', (hook, error) => {
            hook.parent.failed = error;
            hook.parent.eachTest((test) => test.failed = error);
        });
    }

    _injectBeforeEachHookErrorHandling() {
        this._injectHookErrorHandling('beforeEach', (hook, error) => {
            hook.ctx.currentTest.failed = error;
        });
    }

    _injectHookErrorHandling(event, onError) {
        this._addEventHandler(event, this._overrideRunnableFn((runnable) => {
            const baseFn = runnable.fn;

            runnable.fn = function() {
                return runnable.parent.failed
                    ? q.reject(runnable.parent.failed)
                    : q(baseFn).apply(this, arguments).catch((error) => onError(runnable, error));
            };
        }));
    }
